...
heap * heap_new(HEAP_TYPE is_min_heap, int capacity) {
	...
	h->size = capacity;
	h->used = 0;
	h->array = (heap_entry**)malloc(sizeof(heap_entry *) * h->size);
...
static void heap_grow(heap * h) {
	h->array = (heap_entry**)realloc(h->array, sizeof(heap_entry *) * h->size * 2);
	h->size = h->size * 2;
}
...
heap_entry * heap_add(heap * h, int key) {
	...
	// creo nuovo elemento
	heap_entry * e = (heap_entry*)malloc(sizeof(heap_entry));
	e->key = key;
	e->position = h->used;
	h->array[h->used] = e;
